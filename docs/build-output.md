...
发布前可加 `-SkipPublishedCheck` 完成本地文件、实际运行的便携程序、阅读回归和快捷方式检查；上传后必须省略该参数，核对 GitHub 发行文件的校验记录。

此检查只读取文件，核对目录结构、当前和回退 exe 的版本与发行校验值、验证记录、长图截图及桌面快捷方式。需要已构建的本地产物；源码刚克隆时无需执行。
