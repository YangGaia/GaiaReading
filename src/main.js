        ...
        if (DEBUG_OPEN_PATH) {
          ...
          const script = `(async () => {
            try {
              ...
              const epubLayoutWithSearch = __gaiaDebug.getReaderLayoutState();
              const smokeWindowWidth = window.outerWidth;
              const smokeWindowHeight = window.outerHeight;
              window.resizeTo(smokeWindowWidth + 220, smokeWindowHeight + 120);
              await new Promise((r) => setTimeout(r, 700));
              await __gaiaDebug.waitForReaderLayoutRefresh();
              const epubLayoutAfterWindowResize = __gaiaDebug.getReaderLayoutState();
              const epubSearchWindowResizeReady = __gaiaDebug.getBookSearchState().open === true &&
                epubLayoutAfterWindowResize.readerWidth > epubLayoutWithSearch.readerWidth &&
                Math.abs(epubLayoutAfterWindowResize.frameWidth - epubLayoutAfterWindowResize.readerWidth) <= 1 &&
                Math.abs(epubLayoutAfterWindowResize.epubDelta - epubLayoutAfterWindowResize.readerWidth) <= 1;
              window.resizeTo(smokeWindowWidth, smokeWindowHeight);
              await new Promise((r) => setTimeout(r, 700));
              await __gaiaDebug.waitForReaderLayoutRefresh();
              __gaiaDebug.openTocFromEdge();
              const searchSurvivedEdgeToc = __gaiaDebug.getBookSearchState().open === true &&
              ...
              const bookSearchRuntimeReady = bookSearchShortcutState.open === true && bookSearchShortcutState.focused === true &&
                ...
                epubSearchWindowResizeReady === true &&
              ...
              for (let attempt = 0; attempt < 40 && (__gaiaDebug.isSettingsOpen() || __gaiaDebug.getPanels().bookmarksHidden); attempt += 1) {
                await new Promise((resolve) => setTimeout(resolve, 50));
              }
              const countAfterAdd = __gaiaDebug.getBookmarkCount();
              const bookmarkActionClosedSettings = !__gaiaDebug.isSettingsOpen();
              const bookmarkActionOpenedPanel = !__gaiaDebug.getPanels().bookmarksHidden;
              if (bookmarkActionOpenedPanel) __gaiaDebug.togglePanel('bookmarks');
              ...
              const tocButton = document.getElementById('btn-reader-toc');
              const tocButtonReady = !!tocButton && tocButton.offsetWidth > 0 && tocButton.getAttribute('aria-controls') === 'toc-panel';
              const readerFooter = document.querySelector('#reader-view .statusbar');
              const readerContentRect = document.getElementById('reader-content').getBoundingClientRect();
              const readerFooterRect = readerFooter && readerFooter.getBoundingClientRect();
              const bottomControlsClearContent = !!readerFooterRect && tocButton.parentElement === readerFooter && document.getElementById('page-nav').parentElement === readerFooter && readerContentRect.bottom <= readerFooterRect.top + 1;
              __gaiaDebug.openTocFromEdge();
              const tocHoverOpened = __gaiaDebug.getPanels().tocMode === 'hover' && !__gaiaDebug.getPanels().tocHidden;
              if (firstTocLink) firstTocLink.click();
              await new Promise((r) => setTimeout(r, 500));
              const tocHoverStayed = __gaiaDebug.getPanels().tocMode === 'hover' && !__gaiaDebug.getPanels().tocHidden;
          ...
          try {
            ...
            debugOk =
              ...
              parsed.bookmarkActionClosedSettings === true &&
              parsed.bookmarkActionOpenedPanel === true &&
